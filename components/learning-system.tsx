    ...
    <>
      <Tabs defaultValue="modules" className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <TabsList className="grid w-full grid-cols-2 gap-2 rounded-lg p-1 md:w-auto md:max-w-lg">
            <TabsTrigger
              value="modules"
              className="flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-base font-semibold transition-all data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg"
            >
        ...
        <TabsContent value="case-studies">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {caseStudies.map((study) => (
              <CaseStudyCard key={study.id} study={study} onReadMore={() => handleReadMore(study)} />
            ))}
          </div>
        </TabsContent>
      </Tabs>
      <CaseStudyDetail study={selectedStudy} isOpen={isStudyDetailOpen} onOpenChange={setIsStudyDetailOpen} />
    </>
  )
}